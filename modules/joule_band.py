...
def check_mod_enhancements():
    ...
    if joule_data.GameSource == "rb3":
        if len(get_data_indexes("trackNotesOn", "PART DRUMS", "95")) > 0:
            output_add(
                "messages",
                f'Expert+ kick notes found on Drums. This song is compatible with the "Double Bass Pedal" modifier in RB3Enhanced.',
            )
            output_add("flags", "drums_has_2x")

            if "PART DRUMS_2X" not in joule_data.TracksFound:
                joule_print(
                    'Expert+ Kick notes found on "PART DRUMS", PART DRUMS_2X will be created to process this.'
                )
                ...
                joule_data.GameData["trackNotesOn"].update(
                    extract_data("trackNotesOn", "PART DRUMS", "PART DRUMS_2X")
                )
                joule_data.GameData["trackNotesOff"].update(
                    extract_data("trackNotesOff", "PART DRUMS", "PART DRUMS_2X")
                )
                joule_data.GameData["trackNotesMeta"].update(
                    extract_data("trackNotesMeta", "PART DRUMS", "PART DRUMS_2X")
                )
                ...
                joule_print(
                    'Expert+ Kick notes found on "PART DRUMS". Ignoring, because PART DRUMS_2X was found.'
                )
            pass
