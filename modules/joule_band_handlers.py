...
def format_location(note_location: int, display_time=False):
    ...
    for test in range(location_amount):
        ...
        if note_location >= time_signature_locations[location_index]:
            ...
            location_num = get_meta(
                "time_signature_num", joule_band.time_signature_time[location_index]
            )
            location_denom = get_meta(
...
def generate_seconds():
    ...
    if get_meta("TotalLength") == None:
        joule_print(
            "generate_seconds: File has not been initialized for seconds generation!"
        )
        return False
...
def format_seconds(note_location: int):
    ...
    if get_meta("TotalLength") == None:
        joule_print(
            "format_seconds: File has not been initialized for seconds generation!"
        )
        return "SecondsError"
...
def process_time_signature(ticks: int, numerator: int, denominator: int):
    ...
    ticks_per_measure = (
        joule_data.TicksPerBeat / joule_band.last_time_signature_denom
    ) * joule_band.last_time_signature_num

    tempPosition = (tempPosition / ticks_per_measure) / 4

    joule_band.time_signature_last_measure = (
        joule_band.time_signature_last_measure + tempPosition
    )
